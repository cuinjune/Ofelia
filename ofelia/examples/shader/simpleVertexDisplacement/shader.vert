...
// the time and scaleFactor value is passed into the shader by the OF app.
uniform float time; // ofGetElapsedTime
uniform float scaleFactor; // ofGetRetina * ofGetWindowScale
...
void main()
{
    ...
    float displacementHeight = 50.0 * scaleFactor;
    float displacementY = sin(time + (position.x / (50.0 * scaleFactor))) * displacementHeight;
