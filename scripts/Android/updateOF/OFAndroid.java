...
//		WifiManager wifi = (WifiManager)ofActivity.getSystemService( Context.WIFI_SERVICE ); //OFELIANOTE : replaced with below
        WifiManager wifi = (WifiManager)ofActivity.getApplicationContext().getSystemService( Context.WIFI_SERVICE );
		if(wifi != null)
		{
	...
	public static boolean keyDown(int keyCode, KeyEvent event) {
        if ((keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0)) {
            if( onBackPressed() ) {
				...
				android.os.Process.killProcess(android.os.Process.myPid()); //OFELIANOTE : added this code
           		return false;
           	}
        }
